...
def finite_difference_jacobian(model, x, u, epsilon=1e-4):
    """
    Numerically approximate the Jacobians A (df/dx) and B (df/du) via finite differences.
    
    Arguments:
        model: the dynamics model with a .predict(x, u) function.
        x: current state, array-like with shape (n,)
        u: current control, array-like with shape (m,)
        epsilon: small perturbation value.
    
    Returns:
        A: Jacobian with respect to state, shape (output_dim, n)
        B: Jacobian with respect to control, shape (output_dim, m)
    """
    # Evaluate base output
    base = model.predict(x, u)
    output_dim = base.shape[0]
    n = x.shape[0]
    m = u.shape[0]
    
    A = np.zeros((output_dim, n))
    for i in range(n):
        dx = np.zeros_like(x)
        dx[i] = epsilon
        perturbed = model.predict(x + dx, u)
        A[:, i] = (perturbed - base) / epsilon

    B = np.zeros((output_dim, m))
    for i in range(m):
        du = np.zeros_like(u)
        du[i] = epsilon
        perturbed = model.predict(x, u + du)
        B[:, i] = (perturbed - base) / epsilon

    return A, B

class iLQRController:
    def __init__(self, dynamics_model, horizon=50, max_iter=10, 
                 Q_terminal=np.diag([1000, 0]), R=0.01):
    ...
    def dynamics(self, x, u):
        """
        Uses the learned dynamics model and approximates linearization using finite differences.
        Returns: predicted next state x_next, and Jacobians A (∂f/∂x) and B (∂f/∂u).
        """
        x_next = self.dynamics_model.predict(x, u)
        A, B = finite_difference_jacobian(self.dynamics_model, x, u, epsilon=1e-4)
        return x_next, A, B
    ...
    def backward_pass(self, x_seq, u_seq, A_list, B_list):
        Vx = self.Q_terminal @ (x_seq[-1] - np.array([TARGET_POSITION, 0]))
        Vxx = self.Q_terminal
    ...
    def forward_pass(self, x0, u_seq, k, K):
        ...
        total_cost += 0.5 * (x - np.array([TARGET_POSITION, 0])).T @ self.Q_terminal @ (x - np.array([TARGET_POSITION, 0]))
...
def main():
    ...
    u_guess = np.random.uniform(-1, 1, (controller.horizon, 1))  # Random initial guess
    
    # Phase 1: Random Exploration
    exploration_steps = 50  # Variable exploration phase
